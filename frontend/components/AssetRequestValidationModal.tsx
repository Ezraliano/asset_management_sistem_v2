...
const AssetRequestValidationModal: React.FC<AssetRequestValidationModalProps> = ({
  ...
  const fetchAvailableAssets = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/asset-requests-available-assets', {
        headers: {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/asset-requests/${request.id}/approve`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleReject = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`http://127.0.0.1:8000/api/asset-requests/${request.id}/reject`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const handleReturnAsset = async (formData: FormData) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`http://127.0.0.1:8000/api/asset-requests/${request.id}/return`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const handleConfirmReturn = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`http://127.0.0.1:8000/api/asset-requests/${request.id}/confirm-return`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleRejectReturn = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`http://127.0.0.1:8000/api/asset-requests/${request.id}/reject-return`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          {request.status === 'APPROVED' && request.loan_photo_path && (
            <div className="mt-4">
              ...
              <div className="mt-2">
                <img
                  src={`http://127.0.0.1:8000/storage/${request.loan_photo_path}`}
        ...
        <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          {request.return_proof_photo_path && (
            <div className="mb-3">
              ...
              <div className="mt-2">
                <img
                  src={`http://127.0.0.1:8000/storage/${request.return_proof_photo_path}`}
